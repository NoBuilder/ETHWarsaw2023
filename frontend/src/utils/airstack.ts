...
async function getSocialMediaInfo(ethAddresses: string[]): Promise<SocialMediaInfo[]> {
    ...
    return results.filter(result => result !== null) as SocialMediaInfo[];
}

async function getProfileFromDapp(ethAddress: string, dappName: string): Promise<SocialMediaInfo | null> {
    const query = `
        query {
